...
function csvRowToObj(headers: string[], row: string[]): Record<string, string> {
  ...
  for (let c = 0; c < headers.length; c++)
    obj[headers[c] || `col_${c + 1}`] = String(row[c] ?? "");
  return obj;
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (intent === "order_details") {
      ...
      if (!orderId)
        return jsonResponse(
          { ok: false, intent, error: "Missing order_id" } satisfies ActionErr,
          400,
        );
        ...
        return jsonResponse(
          {
            ok: false,
            intent,
            error: "Shopify GraphQL error while loading order details.",
            details: JSON.stringify(json.errors, null, 2),
          } satisfies ActionErr,
          500,
        );
      }
      ...
      const fulfillmentOrders = (order.fulfillmentOrders?.nodes || []).map(
        (fo: any) => ({
          id: String(fo.id),
    ...
    if (intent === "create_fulfillments_grouped") {
      ...
      const pickedIds = Array.from(form.keys())
        .filter(
          (k) => k.startsWith("pick_") && String(form.get(k) || "") === "1",
        )
        ...
        return jsonResponse(
          {
            ok: false,
            intent,
            error: "Please pick at least 1 item.",
          } satisfies ActionErr,
          400,
        );
      }
      ...
      const items: PickedItem[] = pickedIds
        .map((foLineItemId) => {
          const fulfillmentOrderId = String(
            form.get(`fo_${foLineItemId}`) || "",
          );
          const qty = Math.max(
            0,
            Number(String(form.get(`qty_${foLineItemId}`) || "0")),
          );
          const trackingNumber = String(
            form.get(`tn_${foLineItemId}`) || "",
          ).trim();
        ...
        return jsonResponse(
          {
            ok: false,
            intent,
            error: "Quantity must be > 0 for picked items.",
          } satisfies ActionErr,
          400,
        );
      }
      ...
      for (const it of items) {
        const key = it.trackingNumber
          ? `${it.trackingNumber}|||${it.carrier || ""}`
          : "NO_TRACK";
      ...
      for (const [key, groupItems] of groupMap.entries()) {
        ...
        const lineItemsByFulfillmentOrder = Array.from(byFO.entries()).map(
          ([fulfillmentOrderId, foLineItems]) => ({
            fulfillmentOrderId,
            fulfillmentOrderLineItems: foLineItems.map((x) => ({
              id: x.id,
              quantity: x.quantity,
            })),
          }),
          ...
          return jsonResponse(
            {
              ok: false,
              intent,
              error: errs[0]?.message || "Fulfillment failed",
              details: JSON.stringify(errs, null, 2),
            } satisfies ActionErr,
            400,
          );
        }
    ...
    if (intent === "csv_sync") {
        ...
        return jsonResponse(
          {
            ok: false,
            intent,
            error: "CSV seems empty or invalid (no header row found).",
          } satisfies ActionErr,
          400,
        );
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const trackingNumber = pickHeader(obj, [
          "tracking_number",
          "tracking",
          "tn",
        ]);
        const carrier = pickHeader(obj, [
          "carrier",
          "company",
          "shipping_company",
        ]);
        ...
        if (!normalized) {
          if (errorsSample.length < 20)
            errorsSample.push({ row: rowNum, error: "Missing order_name" });
          continue;
        }
        ...
        try {
          ...
          if (!fos.length) {
            if (errorsSample.length < 20)
              errorsSample.push({
                row: rowNum,
                order_name: normalized,
                error: "No fulfillmentOrders (not shippable / no location)",
              });
            continue;
          }
          ...
          const trackingInfo =
            String(trackingNumber || "").trim().length > 0
              ? {
                  number: String(trackingNumber).trim(),
          ...
          for (const fo of fos) {
            ...
            const lineItems = liNodes
              .map((li: any) => ({
                id: String(li.id),
                quantity: Number(li.remainingQuantity ?? 0),
              }))
            ...
            if (json3?.errors?.length || errs.length) {
              const msg = json3?.errors?.length
                ? "GraphQL error"
                : errs[0]?.message || "Fulfillment failed";
              if (errorsSample.length < 20)
                errorsSample.push({
                  row: rowNum,
                  order_name: normalized,
...
function buildInitialItemState(
  details: OrderDetailsOk,
...
export default function FulfillmentCenterPage() {
  ...
  const orders: OrderListItem[] =
    loader && (loader as any).ok ? (loader as any).orders : [];

  // Bulk CSV sync fetcher
  const bulkFetcher = useFetcher<AnyFetcherData>();
  const bulkOk =
    bulkFetcher.data &&
    (bulkFetcher.data as any).ok === true &&
    (bulkFetcher.data as any).intent === "csv_sync"
      ? (bulkFetcher.data as CsvSyncOk)
      : null;
  const bulkErr =
    bulkFetcher.data &&
    (bulkFetcher.data as any).ok === false &&
    (bulkFetcher.data as any).intent === "csv_sync"
  ...
  const [detailsById, setDetailsById] = useState<
    Record<string, OrderDetailsOk>
  >({});
  const [itemsByOrderId, setItemsByOrderId] = useState<
    Record<string, Record<string, ItemState>>
  >({});
  ...
  const createOk =
    createFetcher.data &&
    (createFetcher.data as any).ok === true &&
    (createFetcher.data as any).intent === "create_fulfillments_grouped"
  ...
  const mergedItems = useMemo(() => {
    ...
    type M =
      OrderDetailsOk["fulfillmentOrders"][number]["lineItems"][number] & {
        _foId: string;
        _foStatus: string;
        _key: string;
      };
          ...
          <Card>
            <BlockStack gap="300">
              ...
              <Text as="p" tone="subdued">
                Upload CSV and click Sync. Suggested columns:{" "}
                <code>order_name</code>, <code>tracking_number</code>,{" "}
                <code>carrier</code>, <code>notify_customer</code>.
              </Text>
              ...
              {bulkOk ? (
                <Banner tone="success" title="CSV Synced">
                  <p>
                    Processed <b>{bulkOk.processed}</b> / {bulkOk.totalRows}{" "}
                    rows · created fulfillments:{" "}
                    <b>{bulkOk.createdFulfillments}</b> · failed:{" "}
                    <b>{bulkOk.failed}</b>
                  </p>
                    ...
                    <>
                      ...
                      <ul>
                        {bulkOk.errorsSample.map((e, idx) => (
                          <li key={idx}>
                            Row {e.row}{" "}
                            {e.order_name ? `(${e.order_name})` : ""}: {e.error}
                          </li>
                        ))}
                      </ul>
                    </>
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="300">
              ...
              <Text as="p" tone="subdued">
                Danh sách bên dưới là các order <b>Unfulfilled / Partial</b>.
                Bấm Open để xổ ngay dưới dòng đó và nhập tracking theo từng
                item. (Nếu tracking khác nhau, Shopify sẽ tạo nhiều fulfillment
                theo nhóm tracking.)
              </Text>
              ...
              {createOk ? (
                <Banner tone="success" title="Fulfillment(s) created">
                  <p>
                    Created <b>{createOk.created}</b> fulfillment(s) for{" "}
                    <code>{createOk.orderId}</code>
                  </p>
                  <ul>
                    {createOk.results.map((r, i) => (
                      <li key={i}>
                        [{r.key}] id: <code>{r.fulfillmentId || ""}</code> ·
                        status: <b>{r.status || ""}</b>
                      </li>
                    ))}
                  </ul>
              ...
              >
                {orders.map((o, idx) => {
                  ...
                  const isLoading =
                    isOpen && detailsFetcher.state !== "idle" && !details;
                          ...
                          <IndexTable.Cell colSpan={5}>
                            ...
                            >
                              {isLoading ? (
                                ...
                                <Banner
                                  tone="critical"
                                  title="Load order details failed"
                                >
                                ...
                                <BlockStack gap="300">
                                  <Card>
                                    <BlockStack gap="200">
                                      {details.fulfillmentOrders.length ===
                                      0 ? (
                                        <Banner
                                          tone="warning"
                                        ...
                                        >
                                          <p>
                                            Order này không có
                                            fulfillmentOrders. Thường là: item
                                            không requires shipping, hoặc không
                                            có location fulfill/assigned.
                                          </p>
                                        </Banner>
                                      ) : null}
                                      <InlineStack align="space-between">
                                        <Text as="p" tone="subdued">
                                          ...
                                          {details.fulfillmentOrders.length} —
                                          Items:{" "}
                                          {details.fulfillmentOrders.reduce(
                                            (s, fo) => s + fo.lineItems.length,
                                            0,
                                          )}
                                        </Text>
                                        <InlineStack gap="200">
                                          <Badge tone="info">
                                            {details.order.financialStatus ||
                                              "—"}
                                          </Badge>
                                          <Badge
                                            tone={statusTone(
                                      ...
                                      <Text as="p" tone="subdued">
                                        Tip: Tick item + set Qty &gt; 0.
                                        Tracking có thể khác nhau theo item (app
                                        sẽ tự group).
                                      </Text>
                                    </BlockStack>
                                  </Card>
                                  ...
                                  <createFetcher.Form method="post">
                                    <input
                                      type="hidden"
                                      name="intent"
                                      value="create_fulfillments_grouped"
                                    />
                                    <input
                                      type="hidden"
                                      name="order_id"
                                    ...
                                    <Card>
                                      <BlockStack gap="300">
                                        ...
                                        <Text as="p" tone="subdued">
                                          Nếu bạn nhập tracking khác nhau cho
                                          từng item, app sẽ tự group theo
                                          tracking và tạo nhiều fulfillments.
                                        </Text>
                                      </BlockStack>
                                    </Card>
                                  </createFetcher.Form>
